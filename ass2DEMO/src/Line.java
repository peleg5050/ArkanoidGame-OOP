...
public class Line {
    ...
    public boolean isPointInLine(Point startLine, Point endLine, Point point) {
        ...
        return Math.abs((startLine.distance(point) + point.distance(endLine)) - startLine.distance(endLine)) < epsilon;
    }
     ...
     */
    public boolean isParallelYIntersect(Line lineNotParallel, Point lineMinXPoint, Point lineMaxXPoint) {
        ...
        // to the Y axis
        if ((this.start.getX() >= lineMinXPoint.getX()) && (this.start.getX() <= lineMaxXPoint.getX())) {
     ...
     * @return 'true' if the lines intersect, false otherwise.
     */
    public boolean isParallelXIntersect(Line lineNotParallel, Point lineMinYPoint, Point lineMaxYPoint) {
        double xIntersect, m2, b2, yIntersect;
        // If the Y value of this line is between the minimum and the maximum Y value in the line that doesn't parallel
        if ((this.start.getY() >= lineMinYPoint.getY()) && (this.start.getY() <= lineMaxYPoint.getY())) {
    ...
    public boolean isIntersecting(Line other) {
        ...
        // Save the point with the maximum X for 'this' line
        thisMaxXPoint = this.start.xBigger(this.end);
        // Save the point with the minimum X for 'this' line
        thisMinXPoint = this.start.xSmaller(this.end);
        // Save the point with the maximum X for the 'other' line
        otherMaxXPoint = other.start.xBigger(other.end);
        // Save the point with the minimum X for the 'other' line
        otherMinXPoint = other.start.xSmaller(other.end);
        // Save the point with the maximum Y in 'this' line
        thisMaxYPoint = this.start.yBigger(this.end);
        // Save the point with the minimum Y in 'this' line
        thisMinYPoint = this.start.ySmaller(this.end);
        // Save the point with the maximum Y for the 'other' line
        otherMaxYPoint = other.start.yBigger(other.end);
        // Save the point with the minimum Y for the 'other' line
        ...
        if (isThisPoint && isOtherPoint) {
         ...
         // If the 'other' line is actually a point and 'this' line isn't
            ...
            // Check if the point contained in 'this' line
        ...
        // If just 'this' line is parallel to the Y axis
            ...
            return this.isParallelYIntersect(other, otherMinXPoint, otherMaxXPoint);
         // if just the 'other' line is parallel to the Y axis
        } else if (isParallelY(other.start, other.end)) {
            // Check if the lines intersect (using "isParallelYIntersect"): if yes return true, else return false
            return other.isParallelYIntersect(this, thisMinXPoint, thisMaxXPoint);
         // if just 'this' line is parallel to the X axis
            ...
            return this.isParallelXIntersect(other, otherMinYPoint, otherMaxYPoint);
         // if just the 'other' line is parallel to the X axis
            ...
            // Check if the lines intersect (using "isParallelXIntersect"): if yes return true, else return false
            return other.isParallelXIntersect(this, thisMinYPoint, thisMaxYPoint);
        }
    ...
    public Point closestIntersectionToStartOfLine(Rectangle rect) {
        ...
        List<Point> listPointsIntersection = new ArrayList<Point>();
